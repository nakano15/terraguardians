            ...
            if (CanPrayHere && !companion.IsRunningBehavior && companion.Owner == null && Main.rand.NextBool(6))
            {
                ...
                if (companion.Owner == null)
                {
                    ...
                    else
                    {
                        ...
                        for(int n = 0; n < 200; n++)
                        {
                            if (Main.npc[n].active && (Main.npc[n].boss || Terraria.ID.NPCID.Sets.ShouldBeCountedAsBossForBestiary[Main.npc[n].type]) && !NpcMod.IsTowerBoss(Main.npc[n]) && (ReferedPlayer.Center - Main.npc[n].Center).Length() < 2000f)
                            {
                                AnyBoss = true;
                                break;
                            }
